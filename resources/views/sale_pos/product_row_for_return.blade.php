<tr class="product_row" data-row_index="{{$row_count}}">
	...
	@if( ($edit_price || $edit_discount) && empty($is_direct_sell) )
		...
		<input type="hidden" 
			class="product_type" 
			name="exchange_products[{{$row_count}}][product_type]" 
		...
		@if(!empty($discount))
			{!! Form::hidden("exchange_products[$row_count][discount_id]", $discount->id); !!}
		@endif
		...
		@if(!empty($product->lot_numbers))
			<select class="form-control lot_number input-sm" name="exchange_products[{{$row_count}}][lot_no_line_id]" @if(!empty($product->transaction_sell_lines_id)) disabled @endif>
	...
	@if(!empty($is_direct_sell))
  		<br>
  		<textarea class="form-control" name="exchange_products[{{$row_count}}][sell_line_note]" rows="2">{{$sell_line_note}}</textarea>
	...
	<td>
		...
		@if(!empty($product->transaction_sell_lines_id))
			<input type="hidden" name="exchange_products[{{$row_count}}][transaction_sell_lines_id]" class="form-control" value="{{$product->transaction_sell_lines_id}}">
		@endif
		...
		<input type="hidden" value="{{$product->enable_stock}}" 
			name="exchange_products[{{$row_count}}][enable_stock]">
		...
		<input type="text" data-min="1" 
			...
			value="{{@format_quantity($product->quantity_ordered)}}" name="exchange_products[{{$row_count}}][quantity]" data-allow-overselling="@if(empty($pos_settings['allow_overselling'])){{'false'}}@else{{'true'}}@endif" 
			@if($allow_decimal) 
				data-decimal=1 
			@else 
				data-decimal=0 
		...
		<input type="hidden" name="exchange_products[{{$row_count}}][product_unit_id]" value="{{$product->unit_id}}">
		@if(count($sub_units) > 0)
			<br>
			<select name="exchange_products[{{$row_count}}][sub_unit_id]" class="form-control input-sm sub_unit">
                @foreach($sub_units as $key => $value)
		...
		<input type="hidden" class="base_unit_multiplier" name="exchange_products[{{$row_count}}][base_unit_multiplier]" value="{{$multiplier}}">
		...
		@if($product->product_type == 'combo'&& !empty($product->combo_products))
			...
			@foreach($product->combo_products as $k => $combo_product)
				...
				<input type="hidden" 
					name="exchange_products[{{$row_count}}][combo][{{$k}}][product_id]"
					value="{{$combo_product['product_id']}}">

					<input type="hidden" 
					name="exchange_products[{{$row_count}}][combo][{{$k}}][variation_id]"
					value="{{$combo_product['variation_id']}}">

					<input type="hidden"
					class="combo_product_qty" 
					name="exchange_products[{{$row_count}}][combo][{{$k}}][quantity]"
					data-unit_quantity="{{$combo_product['qty_required']}}"
					value="{{$qty_total}}">

					@if(isset($action) && $action == 'edit')
						<input type="hidden" 
							name="exchange_products[{{$row_count}}][combo][{{$k}}][transaction_sell_lines_id]"
	...
	@if(!empty($is_direct_sell))
		<td @if(!auth()->user()->can('edit_product_price_from_sale_screen')) hide @endif">
			<input type="text" name="exchange_products[{{$row_count}}][unit_price]" class="form-control pos_unit_price input_number mousetrap" value="{{@num_format(!empty($product->unit_price_before_discount) ? $product->unit_price_before_discount : $product->default_sell_price)}}">
		</td>
		<td @if(!$edit_discount) hide @endif>
			{!! Form::text("exchange_products[$row_count][line_discount_amount]", @num_format($discount_amount), ['class' => 'form-control input_number row_discount_amount']); !!}<br>
			{!! Form::select("exchange_products[$row_count][line_discount_type]", ['fixed' => __('lang_v1.fixed'), 'percentage' => __('lang_v1.percentage')], $discount_type , ['class' => 'form-control row_discount_type']); !!}
			@if(!empty($discount))
		...
		<td class="text-center {{$hide_tax}}">
			{!! Form::hidden("exchange_products[$row_count][item_tax]", @num_format($item_tax), ['class' => 'item_tax']); !!}
		
			{!! Form::select("exchange_products[$row_count][tax_id]", $tax_dropdown['tax_rates'], $tax_id, ['placeholder' => 'Select', 'class' => 'form-control tax_id'], $tax_dropdown['attributes']); !!}
		</td>
		@if(!empty($warranties))
			{!! Form::select("exchange_products[$row_count][warranty_id]", $warranties, $warranty_id, ['placeholder' => __('messages.please_select'), 'class' => 'form-control']); !!}
		@endif
	@endif
