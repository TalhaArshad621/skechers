$(document).ready(function () {
    ...
    register_report_table = $('#register_report_table').DataTable({
        ...
        ajax: {
            ...
            data: function (d) {
                ...
                if ($('#register_report_date_range').val()) {
                    start = $('input#register_report_date_range')
                        .data('daterangepicker')
                        .startDate.format('YYYY-MM-DD');
                    end = $('input#register_report_date_range')
                        .data('daterangepicker')
    ...
    if ($('#sr_date_filter').length == 1) {
        ...
        sr_sales_commission_report = $('table#sr_sales_with_commission_table').DataTable({
            ...
            fnDrawCallback: function (oSettings) {
                $('#footer_total_invoices').text(
                    sum_table_col($('#sr_sales_with_commission_table'), 'total_invoices')
                );

                $('#footer_total_items').text(
                    sum_table_col($('#sr_sales_with_commission_table'), 'total_items')
                );

                $('#footer_total_sales').text(
                    sum_table_col($('#sr_sales_with_commission_table'), 'total_sales')
                );
        ...
        //Sales representative report -> International Exchange
        sr_sales_with_commission_int_exchange = $('table#sr_sales_with_commission_int_exchange_table').DataTable({
        processing: true,
        serverSide: true,
        aaSorting: [[0, 'desc']],
        ajax: {
            url: '/reports/employee-int-exchange-report',
            data: function (d) {
                var start = $('input#sr_date_filter')
                    .data('daterangepicker')
                    .startDate.format('YYYY-MM-DD');
        ...
        fnDrawCallback: function (oSettings) {
            $('#footer_total_invoices_exchange_int').text(
                sum_table_col($('#sr_sales_with_commission_int_exchange_table'), 'total_invoices')
            );

            $('#footer_total_items_exchange_int').text(
                sum_table_col($('#sr_sales_with_commission_int_exchange_table'), 'total_items')
            );

            $('#footer_total_sales_exchange_int').text(
                sum_table_col($('#sr_sales_with_commission_int_exchange_table'), 'total_sales')
            );
            __currency_convert_recursively($('#sr_sales_with_commission_int_exchange_table'));
            __currency_convert_recursively($('#sr_sales_with_commission_exchange_table'));
            __currency_convert_recursively($('#sr_sales_with_commission'));
        },
    });
    ...
    brandfolio_report = $('table#brandfolio_report_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: '/reports/brandfolio-report',
            data: function (d) {
                var start = '';
                var end = '';
                var start_time = $('#product_sr_start_time').val();
    ...
    $(document).on('submit', '#sell_payment_report_form', function (e) {
        e.preventDefault();
        updateSellPaymentReport();
    });

    function updateSellPaymentReport() {
...
function updateSalesRepresentativeReport() {
    ...
    sr_sales_with_commission_int_exchange.ajax.reload();
}
